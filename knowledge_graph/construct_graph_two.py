...
class KnowledgeGraphBuilder:
    ...
    def build_knowledge_graph(self):
        """Build knowledge graph with batch processing"""
        try:
            ...
            while processed < total_paragraphs:
                # Use more efficient cursor with sorting
                cursor = self.db.Paragraphs.find() \
                    .skip(processed) \
                    .limit(batch_size) \
                    .hint("_id_")  # Use _id index for efficient skipping

                batch_processed = 0
                for paragraph in tqdm(cursor,
                                      total=min(batch_size, total_paragraphs - processed),
                                      desc=f"Batch {processed // batch_size + 1}"):
                ...
                if processed % 1000 == 0:
                    ...
                    remaining = (total_paragraphs - processed) / rate if rate > 0 else 0

                    self.logger.info(
                        f"Processed {processed:,}/{total_paragraphs:,} paragraphs "
                        f"({processed / total_paragraphs * 100:.1f}%) "
                        f"Rate: {rate:.1f} docs/sec "
                        f"EST remaining: {remaining / 3600:.1f} hours"
                    )

                # Ensure cursor is closed after each batch
                cursor.close()

            self.logger.info("Knowledge graph construction completed successfully")
            self.logger.info(f"Total time: {(time.time() - start_time) / 3600:.1f} hours")
